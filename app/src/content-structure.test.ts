...
test("abbreviations and decoding methods are defined at first use", () => {
  const required = [
    ["en/infrastructure/01-accelerators-networking.qmd", "The @gls-hbm stacks packaged beside the accelerator provide the large device-memory pool"],
    ["zh/infrastructure/01-accelerators-networking.qmd", "封装在加速器旁的 @gls-hbm 提供大容量的设备内存池"],
...
test("early-book glossary first uses are readable without leaving the page", () => {
  const required = [
    ...
    ["zh/orientation/03-borrowed-ideas.qmd", "@gls-arithmetic-coding，也就是把序列概率转换成一条接近最优的比特流"],
    ["en/orientation/03-borrowed-ideas.qmd", "@gls-two-part-code, an accounting that charges for the model description"],
    ["zh/orientation/03-borrowed-ideas.qmd", "@gls-two-part-code，也就是先计入模型描述成本"],
...
test("generative adaptation and reasoning first uses define the method locally", () => {
  const required = [
    ...
    ["zh/generative/01-diffusion-flow-matching.qmd", "@gls-ddpm，也就是 Ho 等人在 2020 年提出的去噪扩散表述"],
    ["en/generative/04-multimodal-models.qmd", "@gls-clip is the paired image-text training recipe"],
    ["zh/generative/04-multimodal-models.qmd", "@gls-clip，也就是让图像与文本成对学习的训练方法"],
    ["en/generative/04-multimodal-models.qmd", "a single-crop @gls-vit emits approximately"],
    ["zh/generative/04-multimodal-models.qmd", "单次裁剪的 @gls-vit 大约会输出"],
    ["en/adaptation/01-sft-peft.qmd", "@gls-qlora keeps the frozen base weights in a 4-bit NormalFloat representation"],
    ["zh/adaptation/01-sft-peft.qmd", "@gls-qlora把冻结的基座权重保存为 4 位 NormalFloat 表示"],
    ...
    ["zh/reasoning/05-training-to-reason.qmd", "@gls-rloo，即 REINFORCE 留一法（REINFORCE Leave-One-Out），把每条完整回答视为一次采样动作"],
  ];
...
test("safety infrastructure and practice first uses explain operational terms locally", () => {
  const required = [
    ...
    ["en/infrastructure/01-accelerators-networking.qmd", "Through @gls-rdma, a network adapter transfers data between registered memory regions"],
    ["zh/infrastructure/01-accelerators-networking.qmd", "@gls-rdma 允许网络适配器直接在已注册的内存区域之间传输数据"],
...
test("audited leftover first uses define the role of the term in place", () => {
  const required = [
    ...
    ["en/infrastructure/01-accelerators-networking.qmd", "@gls-tpu pod, a cluster of Google's TPU chips"],
    ["en/infrastructure/01-accelerators-networking.qmd", "TPU v4 joined its chips with an @gls-ici"],
    ["en/infrastructure/01-accelerators-networking.qmd", "a @gls-gpu cluster may expose several logical meshes"],
    ["zh/infrastructure/01-accelerators-networking.qmd", "@gls-tpu pod，也就是由 Google TPU 芯片组成的集群"],
