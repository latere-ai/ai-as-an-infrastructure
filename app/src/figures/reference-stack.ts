...
const COMPS: readonly Comp[] = [
  ...
  },
  {
    ...
    examples: { en: "agentgateway, Envoy AI Gateway", zh: "agentgateway、Envoy AI Gateway" },
    more: { en: "agentgateway or Envoy AI Gateway in front of remote MCP servers; the same proxy can also carry model traffic.", zh: "放在远程 MCP 服务器前面的 agentgateway 或 Envoy AI Gateway；同一个代理也可以承载模型流量。" },
  ...
  },
  {
    ...
    more: { en: "Hosted: E2B and Vercel Sandbox on Firecracker microVMs, Modal Sandboxes on gVisor. Self-hosted: Firecracker or gVisor on the stack's own nodes.", zh: "托管：基于 Firecracker microVM 的 E2B 和 Vercel Sandbox，基于 gVisor 的 Modal Sandboxes。自托管：在自有节点上运行 Firecracker 或 gVisor。" },
  ...
  },
  {
    ...
    examples: { en: "LiteLLM, agentgateway, Envoy AI Gateway, Portkey, Bifrost; managed: OpenRouter, Cloudflare", zh: "LiteLLM、agentgateway、Envoy AI Gateway、Portkey、Bifrost；托管：OpenRouter、Cloudflare" },
    more: { en: "Self-hosted: LiteLLM, agentgateway, Envoy AI Gateway, Portkey, Bifrost. Managed: OpenRouter, Cloudflare AI Gateway.", zh: "自托管：LiteLLM、agentgateway、Envoy AI Gateway、Portkey、Bifrost。托管：OpenRouter、Cloudflare AI Gateway。" },
  ...
  },
  {
    ...
    more: { en: "vLLM or SGLang behind an OpenAI-compatible endpoint; llm-d or NVIDIA Dynamo for KV-aware routing and disaggregated serving across many replicas.", zh: "对外提供 OpenAI 兼容端点的 vLLM 或 SGLang；副本很多时，用 llm-d 或 NVIDIA Dynamo 做感知 KV 缓存的路由和分离式服务。" },
  ...
  },
  {
    ...
    more: { en: "Kubernetes with Kueue quotas and gang scheduling (Volcano or KAI Scheduler), Slurm, or Ray on KubeRay; rented per second from Modal, Baseten, Fireworks, or Together, or reserved from a neocloud such as CoreWeave or Lambda.", zh: "带 Kueue 配额和成组调度（Volcano 或 KAI Scheduler）的 Kubernetes、Slurm，或运行在 KubeRay 上的 Ray；算力可以按秒从 Modal、Baseten、Fireworks 或 Together 租用，也可以向 CoreWeave、Lambda 等新型云长期预留。" },
