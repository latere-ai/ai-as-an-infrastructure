...
const labels = {
  en: {
    ...
    noteShrunk: "The router knows its error and pulls each estimate toward the batch average before allocating by expected gain; as the error grows, the allocation moves toward uniform. Here it is {r}× as efficient as uniform.",
  ...
  zh: {
    ...
    noteShrunk: "路由器知道自己的误差，先把每个估计向整批平均收缩，再按期望收益分配；误差越大，分配就越趋向均匀分配。此时效率是均匀分配的 {r} 倍。",
