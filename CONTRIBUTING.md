...
| `figures-src/` | Python scripts, one per static chart. Each writes an English and a Chinese SVG to `en/figures/` and `zh/figures/`; `common.py` holds the shared style and the localization |
| `app/` | the reader: a React and Bun static site generator that compiles the `.qmd` sources into `_book/{en,zh}`. The compiler is `app/src/pipeline/`, the browser runtime for runnable cells and interactive figures is `app/src/runtime/`, and chapter review dates live in `app/src/data/review-dates.json` |
...
`@fig-` cross-references, and callout blocks, plus four kinds of figure:

- **Graphviz diagrams** in `` ```{dot} `` blocks, rendered to inline SVG at
  build time. Most diagrams in the book are these.
- **Mermaid diagrams** in `` ```{mermaid} `` blocks, rendered in the browser.
  A plain `` ```mermaid `` fence renders as a code listing, and the lint
  refuses it.
- **Static charts**: an SVG from `figures-src/`, included as an image,
  `![caption](/figures/<name>.svg){#fig-<name>}`.
- **Runnable cells and interactive figures**: `.runnable` blocks that run
  Python in the browser, and `<div class="viz" data-viz="...">` elements
  whose components are registered in `app/src/runtime/viz.ts`.

A Graphviz or Mermaid diagram takes `label` and `fig-cap` directives on its
first lines (`//|` for Graphviz, `%%|` for Mermaid) to become a numbered
figure that `@fig-` can reference; a chart takes its label in the image
attributes.
...
sh tools/lint.sh  # the .qmd lint: em dashes, bare mermaid fences, --- in prose
