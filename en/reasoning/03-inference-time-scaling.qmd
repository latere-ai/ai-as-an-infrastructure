...
correct answer existing and a system finding it. @fig-15-inference-time-scaling-1
...
![Schematic of the coverage-is-not-accuracy gap in parallel test-time scaling. Coverage rises smoothly with the sample count $k$ toward a ceiling; a verifier converts it into accuracy almost one-for-one, while a heuristic selector (majority vote or a reward model) plateaus past hundreds of samples. Idealized curves, after Brown et al. (2024).](/figures/15-inference-time-scaling-1.svg){#fig-15-inference-time-scaling-1}
...
frozen scalar head, GenRM trains the verifier with the same next-token objective
...
Process reward models, trained in "Let's Verify Step by Step" and used as the
dense verifier in @sec-training-to-reason, are what makes verifier-guided search
...
improvement. @fig-15-inference-time-scaling-2 shows both regimes: a perfect
...
![Schematic of best-of-$n$ against verifiers of differing quality. A perfect verifier rises monotonically toward the coverage ceiling, so more samples never hurt; an imperfect verifier peaks at some optimal $n$ and then declines as the search optimizes against the selector, the inference-time twin of reward hacking. Idealized curves, after Snell et al. (2024) and Brown et al. (2024).](/figures/15-inference-time-scaling-2.svg){#fig-15-inference-time-scaling-2}
