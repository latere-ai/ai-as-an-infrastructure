...
the faults that never trip the crash detector are the ones that have to be
caught by observability or are beyond recovery entirely.
...
checkpoint, not derived fresh on restart, as @fig-orchestration-dataplane shows.
Without that, a resume re-feeds or skips data and silently breaks the mixture
contract.
...
across frontier stacks. @fig-orchestration-checkpoint-tiers lays the two camps
side by side on the axes that separate them.
