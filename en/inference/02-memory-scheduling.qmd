...
A serving engine spends most of its accelerator memory on one growing object,
the key-value cache, and most of its scheduling effort deciding which requests
share a forward pass. The mechanisms in this chapter form a dependency chain.
Continuous batching removes static-batch waste but makes allocation churn visible.
PagedAttention answers it, clearing the fragmented cache and making prefix
sharing cheap. With sharing now cheap, radix-tree prefix caching reuses shared
prompts, but prefill and decode still interfere. That last interference is what
phase splitting removes, and the price is a network transfer. Memory layout and scheduling policy are therefore the same serving
...
digraph {
  ...
  waiting [label="Waiting queue"];
  form [label="Form batch from active set"];
  fwd [label="One forward pass, advance each request by one token"];
  sample [label="Sample one token per request"];
  done [label="Emitted stop token?", shape=diamond];
  retire [label="Retire request, free its blocks"];
  waiting -> form [label="admit"];
  form -> fwd -> sample -> done;
  done -> retire [label="yes"];
  done -> form [label="no, keep in active set"];
  retire -> form [style=dashed, label="slot reused"];
}
```
...
The second move removes that waste by borrowing virtual memory from operating systems. The insight is that the key-value cache is exactly the kind of object virtual memory was invented for: a per-process allocation that grows unpredictably and must share a fixed physical pool without fragmentation. **@gls-pagedattention** [@kwon2023pagedattention] stores the key-value cache in fixed-size blocks, each holding the keys and values for a fixed number of tokens, and lets a request's logical sequence of blocks map to physically non-contiguous blocks through a block table. The attention kernel is rewritten to gather keys and values through that table instead of assuming contiguity. Now allocation is per block, not per request: a request grows by appending a block when its current block fills, memory is shared at block granularity across all requests, and the only internal waste is the partial final block. The analogy is precise. The block is a page, the block table is a page table, and the engine is a memory manager that hands out and reclaims pages as sequences grow and finish. @fig-memory-scheduling-paging shows the indirection: a request's logical sequence of token blocks maps through a block table to physically scattered blocks in a shared pool.
...
digraph {
  ...
  subgraph cluster_logical {
    label="Logical KV (per request)"; style=rounded; color="#6b7280"; fontcolor="#6b7280"; fontsize=10;
    t0 [label="tokens 0-15"];
    t1 [label="tokens 16-31"];
    t2 [label="tokens 32-47"];
    t0 -> t1 -> t2;
  }
  subgraph cluster_table {
    label="Block table"; style=rounded; color="#6b7280"; fontcolor="#6b7280"; fontsize=10;
    e0 [label="slot 0"];
    e1 [label="slot 1"];
    e2 [label="slot 2"];
  }
  subgraph cluster_phys {
    label="Physical blocks (shared pool)"; style=rounded; color="#6b7280"; fontcolor="#6b7280"; fontsize=10;
    b2 [label="block 2"];
    b7 [label="block 7"];
    b9 [label="block 9"];
  }
...
digraph {
  ...
  prompt [label="Prompt"];
  prompt -> pp;
  subgraph cluster_prefillpool {
    label="Prefill pool (compute-bound)"; style=rounded; color="#6b7280"; fontcolor="#6b7280"; fontsize=10;
    pp [label="Read full prompt, one pass"];
    ft [label="First token"];
    kv [label="Build KV cache"];
    pp -> ft;
    pp -> kv;
  }
  kv -> dp [label="KV cache transfer over network"];
  subgraph cluster_decodepool {
    label="Decode pool (bandwidth-bound)"; style=rounded; color="#6b7280"; fontcolor="#6b7280"; fontsize=10;
    dp [label="Read KV cache"];
    step [label="Emit one token per pass"];
    dp -> step;
    step -> dp [label="append to KV"];
  }
  out [label="Output stream"];
  step -> out;
}
```

This is the most recent move, and it inverts the assumption the first three shared. The within-device line, exemplified by chunked-prefill scheduling [@agrawal2024sarathi], keeps one resource pool and schedules carefully; the disaggregated line [@zhong2024distserve; @qin2024mooncake] accepts a network transfer of the key-value cache to remove interference entirely. At scale the two lines composed rather than one displacing the other: fleets disaggregate across pools while each pool's scheduler still chunks and interleaves, and disaggregation is the core design of the production orchestration layers, NVIDIA Dynamo and the CNCF llm-d project among them [@nvidia2025dynamo; @llmd2025].

Once the cache can cross the network, its placement stops being a single device's problem. Production systems now treat KV blocks as a tiered storage hierarchy of their own: hot blocks in accelerator memory, warm blocks spilled to host DRAM and SSD, and shared stores such as LMCache and Mooncake's disaggregated store keeping them addressable across instances, so a prefix computed on one replica can be reused on another [@lmcache2025; @qin2024mooncake]. Routing follows the cache: a KV-aware scheduler such as Dynamo's sends a request to the replica that already holds its prefix rather than to the least loaded one [@nvidia2025dynamo]. The cache this chapter began by scheduling within one device ends as a placed, routed resource at fleet scale.
