...
$b$ is the bit width, and $s$ is the scale that maps the largest absolute
weight in the block to the largest signed integer representable with $b$ bits.
The idea is to store a small integer grid plus the ruler needed to turn grid
points back into approximate weights, so a block of weights becomes $b$-bit
