...
A second problem compounds the first. Any public benchmark, the moment it is published, begins leaking into training corpora and becomes a target to tune toward [@shi2023detecting]. A number that everyone can see is a number everyone optimizes, and Goodhart's law takes it from there: the measure stops measuring. So the question is not only how to grade open-ended output, but how to hold a measurement that stays honest after the field has seen it.

The chapter answers with three instruments built on top of each other: a model that grades, a ranking that aggregates many grades, and a held-out set that keeps the whole apparatus honest. Holistic evaluation in the sense of HELM, scoring models across many scenarios and metrics at once rather than chasing a single headline number, is the frame these three sit inside [@liang2022holistic].
...
digraph judge_bias {
  ...
  e1 [label="rewards padding", style="rounded,filled", fillcolor="#f5e6cc"];
  e2 [label="rewards label order", style="rounded,filled", fillcolor="#f5e6cc"];
  e3 [label="rewards same-family output", style="rounded,filled", fillcolor="#f5e6cc"];
  e4 [label="rewards surface form", style="rounded,filled", fillcolor="#f5e6cc"];

  m1 [label="normalize length, discount it", style="rounded,filled", fillcolor="#d6e8d5"];
  m2 [label="swap order, average verdicts", style="rounded,filled", fillcolor="#d6e8d5"];
  m3 [label="cross-family judge or ensemble", style="rounded,filled", fillcolor="#d6e8d5"];
  m4 [label="rubric anchoring, pairwise", style="rounded,filled", fillcolor="#d6e8d5"];
