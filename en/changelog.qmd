...
The book is open source, and the site now says so where it matters: a page on what is worth reporting, and a line at the foot of every chapter linking to the file that chapter is written from. A routing fault that stripped images off some addresses was fixed.

**New**

- Contribute, a page on what is most useful to report, how to send a change, and what the licence allows. Every chapter now ends with two links into the repository: one opens an issue about that chapter, the other opens its source file for editing.

**Changed**

- The source repository is reachable from the sidebar and the header of every page. The links above the table of contents no longer scroll out of view when the site opens on the preface.

**Corrected**

- A page opened at the wrong spelling of its address, with a trailing slash where it takes none or without one where it needs one, still showed its text but lost its images and its links to other chapters. Each page now has a single address, and the other spelling redirects to it.
