...
flowchart TB
  R["Multi-agent run"] --> Q{"Did the agents converge?"}
  Q -->|"No, no agreement reached"| L["Liveness failure"]
  Q -->|"Yes, on one answer"| S{"Is the answer correct?"}
  S -->|"Yes"| OK["Ship the answer"]
  S -->|"No, agreed but wrong"| F["Safety failure"]
  L --> ESC["Escalate to human or different model"]
  F --> BLK["Block: silent and would ship"]
```
...
stateDiagram-v2
  [*] --> ClaimTopic: fork critic, no shared context
  ClaimTopic --> Attack: pick a topic not yet taken
  Attack --> Defend: proposer responds
  Defend --> Attack: attack stands, next round
  Defend --> Conceded: proposer concedes
  Defend --> GaveUp: critic gives up
  Attack --> Capped: hard round cap reached
  Conceded --> [*]: apply fix to artifact
  GaveUp --> [*]: drop attack
  Capped --> Unresolved: surface, rank by rounds survived
  Unresolved --> [*]
```
