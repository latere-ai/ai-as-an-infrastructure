...
The grader differs by agent type. For coding agents it runs the test suite and inspects the final tree. For conversational agents it scores the dialogue against a rubric, checks the resulting state, and simulates users with personas under a turn cap [@yao2024taubench]. For tool agents it replays or inspects API state, because a syntactically valid tool call can still be semantically wrong. For research agents it checks groundedness, coverage, and source quality, using the claim-and-evidence discipline of @sec-factuality-grounding. For computer-use agents it reads backend, filesystem, and URL state rather than the confirmation screen the agent says it reached.
