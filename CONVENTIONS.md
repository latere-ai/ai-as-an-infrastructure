...
- Diagrams are Graphviz in `` ```{dot} `` blocks, rendered to inline SVG at
  build time, so they need no script in the reader's browser. The reader also
  renders `` ```{mermaid} `` blocks, client-side; a plain `` ```mermaid ``
  block (no braces) renders as a code listing, not a diagram. Verify a
  diagram renders at the mobile column width, not just that its syntax
  parses.
- A chart drawn from data is a static SVG generated by a script in
  `figures-src/`, which writes the English and Chinese versions together.
