...
ZH_TEXT = {
    ...
    "long-to-short": "长转短",
    "short student": "短链学生",
    # Verifier-taxonomy labels.
    "three independent verifier choices": "验证器的三个独立选择",
    "signal location": "信号位置",
    "terminal outcome": "最终结果",
    "intermediate process": "中间过程",
    "judgment source": "判断来源",
    "explicit rule": "显式规则",
    "learned model": "习得模型",
    "human review": "人工审查",
    "return type": "返回类型",
    ...
    "critique": "评析",
    "certificate": "证书",
    "examples combine one choice from each row": "具体方案从每行各选一项进行组合",
    ...
    "context length L (thousands of tokens)": "上下文长度 L（千词元）",
    "tensor size (GiB)": "张量大小（GiB）",
    ...
    "Indexed example (8 candidates/doc)": "索引示例（每篇文档 8 个候选）",
    ...
    "DPO log-sigmoid (no finite target)": "DPO 对数 S 形损失（无有限目标）",
    ...
    "IPO squared loss (finite target)": "IPO 平方损失（有限目标）",
    "KL drift from SFT reference": "相对 SFT 参考的 KL 漂移",
    "policy shift from reference": "相对参考策略的偏移",
    "score (illustrative units)": "分数（示意单位）",
    "optimized proxy score": "被优化的代理分数",
    "independent gold-model score": "独立金标准模型分数",
    "gold-model peak": "金标准模型峰值",
    "proxy keeps rising": "代理分数继续上升",
    "KL from reference (optimization pressure)": "相对参考模型的 KL（优化压力）",
    "KV cache memory (GB)": "KV 缓存内存（GB）",
    "KV payload (GiB)": "KV 有效载荷（GiB）",
    "KV memory utilization": "KV 内存利用率",
    "MHA": "MHA",
    "MHA (32 KV heads)": "MHA（32 个 KV 头）",
    "GQA (8 KV heads)": "GQA（8 个 KV 头）",
    "MQA (1 KV head)": "MQA（1 个 KV 头）",
    "MLA": "MLA",
    "MQA": "MQA",
    "MCP server": "MCP 服务器",
    ...
    "Parameters in one MoE layer (billions)": "单个 MoE 层参数量（十亿）",
    ...
    "diffusion": "扩散",
    "curved probability-flow path": "弯曲的概率流路径",
    "direct": "直接回答",
    "empirical": "经验验证",
    "distilled": "蒸馏后",
    "draft + edit": "草稿 + 编辑",
    ...
    "tied input / output weights": "输入 / 输出权重绑定",
    "untied input + output weights": "输入 + 输出权重不绑定",
    ...
    "fine-tune": "微调",
    "fixed patches": "固定图块",
    "linear conditional path": "线性条件路径",
    ...
    "patch size 14": "图块边长 14",
    "patch size 16": "图块边长 16",
    "visual patch positions": "视觉图块位置数",
    "10 Hz control": "10 Hz 控制",
    "50 Hz control": "50 Hz 控制",
    "episode duration (seconds)": "回合时长（秒）",
    "control targets": "控制目标数",
    "4096-square matrix": "4096 阶方阵",
    "LoRA rank r": "LoRA 秩 r",
    "LoRA parameters (% of matrix)": "LoRA 参数（矩阵占比 %）",
    "imitation": "模仿",
    ...
    "latency": "延迟",
    "latency budget": "延迟预算",
    "12.5 frames/s": "12.5 帧/秒",
    "50 frames/s": "50 帧/秒",
    "active codebooks Q": "启用码本数 Q",
    "codec indices per second": "每秒编解码器索引数",
    ...
    "model weights": "模型权重",
    "7B weights at 2 bytes / parameter": "7B 权重（每参数 2 字节）",
    "modular": "模块化",
    "minimizes total cost": "块大小",
    "normalized path coordinate": "归一化路径坐标",
    ...
    "token-indexed parameter storage (GiB)": "词元索引参数存储量（GiB）",
    "Pinned parallel text\nsame content": "固定平行文本\n内容相同",
    "One pinned\nnormalization policy": "一种固定的\n规范化策略",
    "One tokenizer\nartifact": "同一个分词器\n产物",
    "Premium distribution\nby language + domain": "按语言 + 领域统计\n词元溢价分布",
    "p_l = tokens_l / tokens_reference": "p_l = 语言词元数 / 参考词元数",
    "Report median, p95, and truncation at the deployed context limit": "报告中位数、p95，以及部署上下文上限下的截断率",
    "too dense:": "过密：",
    "too sparse:": "过疏：",
