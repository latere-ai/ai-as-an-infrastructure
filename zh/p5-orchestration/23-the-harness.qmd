...
@fig-harness-cancel-cascade 追踪了这条级联，以及每一个不配合的跳所添加的延迟，于是
「取消只与它最盲目的那一跳一样真实」这句话，就从一句断言变成了这条链的一个性质。
...
@fig-harness-lock-altitude 直接展示了这种错位。
...
抬高了天花板，却让发现质量成为运行时并不拥有的模型能力的函数。@fig-harness-tool-ladder
把这四级摆在各自买来的扩展天花板与添加的代价旁边。
