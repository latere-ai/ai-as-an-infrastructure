...
模型要和它从未训练过的数据对话，靠的是检索；而决定哪些数据能到达模型面前的，是文档智能这道前门。实践层接在 @sec-rag-retrieval 与 @sec-context-engineering 之下：那两章讲检索为何有效、上下文窗口如何组装，这里处理 2026 年的工具选择与接线。这两半其实是同一条流水线：一份 PDF 变成结构化文本，结构化文本变成向量库里的嵌入，一次查询把对的分块取回来，重排器在它们进入提示词之前排好序。上游任何一处出错，下游的一切都会被悄无声息地毒化。
...
这条流水线从摄取开始。@fig-retrieval-pipeline 画出了从原始文档到回答的整条路径。
...
digraph {
  ...
  subgraph cluster_Ingest {
    label="摄取 离线"; style=rounded; color="#6b7280"; fontcolor="#6b7280"; fontsize=10;
    D [label="文档\nPDF 扫描件、Office 文件"];
    P [label="解析与 OCR\nVLM 或流水线"];
    X [label="结构化抽取\n符合模式的对象"];
    C [label="分块与嵌入"];
    VDB [label="向量库", shape=cylinder];
    D -> P;
    P -> X;
    P -> C;
    X -> VDB;
  ...
  subgraph cluster_Query {
    label="查询 在线"; style=rounded; color="#6b7280"; fontcolor="#6b7280"; fontsize=10;
    Q [label="用户或智能体查询"];
    H [label="混合检索", shape=diamond];
    F [label="RRF 融合"];
    RR [label="重排"];
    CTX [label="Top-k 上下文"];
    LLM [label="生成器或智能体"];
  ...
  EVAL [label="评估 RAGAS 或 LangSmith"];
  EVAL -> H [style=dashed];
