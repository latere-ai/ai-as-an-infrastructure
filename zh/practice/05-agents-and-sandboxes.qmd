...
@sec-agent-architectures 的智能体架构、@sec-the-harness 的运行框架、@sec-security-authorization 的授权模型，在这里汇成一个工程决策：2026 年要发布一个智能体，运行时该选什么、怎样接入。真正有用的选择不是给库排名，而是选定循环原型、工具协议、沙箱边界，以及一个不容易后悔的默认方案。
...
有一条主线贯穿始终：框架、工具、沙箱都把密钥和模型访问留在*外面*，只通过受治理的接缝伸进来。模型走由@gls-gateway签发的@gls-virtual-key，凭据走短时效的受限令牌，网络走默认拒绝加允许列表的出网。把这套纪律做对了，具体选哪个产品就成了可以反悔的决定。
...
::: {.callout-tip}
## 下层约束
框架不可能比它下面的网关层和服务层更可靠（@sec-serving-stack，@sec-wiring-stack）。这正是「持久化执行」和「检查点」会成为 2026 年头牌特性的原因：它们让框架可以从下层故障中恢复。让框架指向一个带虚拟密钥的模型网关，而不是原始的提供方凭据，下层的治理，也就是允许列表、预算上限、模型替换，就变成服务端的一次改动，无需重新部署智能体。
:::
...
安全理由很直接：智能体一旦能执行代码，网关上的越狱过滤器对 `rm -rf`、对挖矿程序、对模型被诱导吐出的外泄 `curl`，统统无能为力。2026 年的安全共识（Firecrawl 等）是：稳健防御依赖代码运行物理环境的隔离，而不是在系统提示里好言相劝。这是 @sec-security-authorization 的实战面：执行边界在运行时，而不在提示。
