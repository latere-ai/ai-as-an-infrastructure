...
这些安全实践在 2026 年的各类来源里反复出现，并共同指向同一条运维纪律：最小权限、短时效凭据；把每个工具或浏览器的输出都当作不受信任的输入，把「思考」和「行动」分开；默认拒绝出网，再配显式白名单；在工具、任务、沙箱生命周期各级设硬超时；不可变的审计日志；默认临时，好让状态和凭据不堆积；以及对不可逆操作设置人在环关卡。框架的选择是可逆的。网关、沙箱和审批这套纪律，才是让它保持可逆的东西。
