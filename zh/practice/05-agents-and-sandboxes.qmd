...
这是一章实践内容。它把 @sec-agent-architectures 的智能体架构、@sec-the-harness 的运行框架、@sec-security-authorization 的授权模型，落到一名工程师在 2026 年真正要做的事情上：怎么选型、怎么接线，把一个智能体发布出去。目标不是把每个库都过一遍，而是给出一套选择的办法，外加一个不容易后悔的默认方案。
...
有一条主线贯穿始终：框架、工具、沙箱都把密钥和模型访问留在*外面*，只通过受治理的接缝伸进来。模型走@gls-gateway虚拟密钥，凭据走短时效的受限令牌，网络走默认拒绝加白名单的出网。把这套纪律做对了，具体选哪个产品就成了可以反悔的决定。
