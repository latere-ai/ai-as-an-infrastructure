...
（`/v1/chat/completions`、`/v1/completions`、`/v1/embeddings`）。正是这一单一约定，让自托管模型可以换成托管模型、多个引擎前可以统一架一个网关，也让评测工具链（@sec-eval-practice）指向与生产相同的 URL。它旁边正浮现第二种形态：OpenAI 自己的主接口已换成有状态的 Responses API，vLLM 也在经典端点之外提供了 `/v1/responses`；但 Chat Completions 仍是各引擎与网关互操作的最小公分母。围绕它来构建，并让每一层都保持可替换。
