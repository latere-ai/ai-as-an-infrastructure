...
服务为何困难，理论部分 @sec-serving-problem 与 @sec-memory-scheduling 已经讲过；落到实践里，它是一条由三个决定组成的请求路径。自托管端点只在负载值得时才划算，@gls-gateway 要放在前面管住密钥与成本，整套东西还必须落到能够承担的算力之上。这条路径自上而下穿过三个层：与之对话的网关、解码 token 的服务引擎，以及下方的 GPU。
...
模型网关是一个控制平面，夹在应用代码和它要对话的众多模型供应商之间。最薄的定义是反向代理：讲一套 API，把请求扇出到 OpenAI、Anthropic、Google、Bedrock、Azure、自建的 vLLM，以及像 OpenRouter 这样的聚合器。但生产职责远不止代理转发。五件事不适合散落在应用代码各处，网关正是把它们集中起来的地方：
...
这套服务最终都会落到一块 GPU 上，需要有人去预置、调度、付费。两个问题居于中心，且大体正交：GPU 从哪里来（**自建对租用**轴）以及它们如何被调度。硬件本身是 @sec-accelerators-networking；编排基底是 @sec-orchestration-data-infra；成本算术是 @sec-economics。这里我们做选择。
