...
**评测（Evaluation）**问的是「输出好不好？」，对照自己定义的标准。它既离线运行（在精挑细选的数据集上、在 CI 里、上线之前），也在线运行（从生产流量中采样、持续打分）。工作的基本单位是一个*分数*。分数有三类，通常混着用：
...
裁判大约比人工评审便宜 500 到 5000 倍，且能逼近人与人之间的一致度，*但只有在校准过之后*。一个没校准过的裁判，会信心十足地打分，也同样信心十足地出错，外部却很难看出来。截至 2026 年年中，@sec-judging-holistic 深入展开的共识做法是：
