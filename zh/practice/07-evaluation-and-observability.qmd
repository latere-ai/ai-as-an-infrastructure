...
这两半之所以相连，是因为**评测的对象，正是所观测到的那条轨迹**。一条生产轨迹会变成一个测试用例。一个裁判在采样的 span 上运行，产出在线分数。仪表盘里抓到的一次回归，会变成离线套件里的一个固定用例。这个循环先观测、再打分、再筛选、再重测，正是「持续评测一个 LLM 应用」的实践内核，本章后续围绕它展开（@fig-eval-loop）。
...
::: {.callout-tip}
## 下层约束
服务层决定了评测套件的形态。因为评测与裁判都指向**模型网关**（@sec-wiring-stack）而非直连提供商，两个模型之间的 A/B 就成了一条路由规则而非一次代码改动，裁判凭据也成了自带预算的受限虚拟密钥而非裸的提供商密钥。在 @sec-serving-stack 选定的网关，正是让持续在线评测运维起来很便宜的原因。
:::
...
裁判大约比人工评审便宜 500 到 5000 倍，且能逼近人与人之间的一致度，*但只有在校准过之后*。一个没校准过的裁判，会一边给出高分一边错得很稳，外部却很难看出来。截至 2026 年年中，@sec-judging-holistic 深入展开的共识做法是：
...
<figcaption>两个裁判给同一批条目打分，准确率保持固定。把基率拖向几乎全是「好」，会出现原始一致度居高不下、而 Cohen's κ 却塌掉的情形，因为当几乎一切都是「好」时，一致大多来自偶然。这正是为什么一个九成时候都同意的裁判，仍可能几乎什么都没说明。示意性。</figcaption>
</figure>
...
循环经由模型网关（@sec-wiring-stack）闭合。被测应用、评测与裁判都调用网关，绝不直连提供商。单是这个决定，就让换模型成了一次配置改动，也让裁判凭据保持为带预算上限的虚拟密钥。
...
这套循环只有在同时改变三条运维轴时才值得搭建。可观测性把非确定性行为变成可调试的东西，从而提升能力；评测把「感觉更好」变成一道发布闸门。网关让在线裁判便宜到可以持续运行，而确定性断言在裁判调用发生之前就拦下成本低、也更该提前发现的故障。信任则落在校准与可携带性上：一个 LLM 裁判可信到何种程度，取决于它对照人类的校准，而 OTel 基座正是让轨迹与分数能从任何变更条款的厂商那里带走的东西。只有这三者都成立，这套循环才值得搭建；而在 2026 年年中，工具终于成熟到让它们能够成立。
